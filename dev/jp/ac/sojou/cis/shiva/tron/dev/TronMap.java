...
class TronMap {
	...
	public void writeToStream(OutputStream outputstream) throws Exception {
		try {
			String s = (new StringBuilder()).append("").append(width).append(
			...
			System.err.println((new StringBuilder()).append(
					"FATAL ERROR: failed to write to stream: ").append(
					exception).toString());
			...
			try {
				...
				if (as.length != 2) {
					System.err
							.println((new StringBuilder())
									.append(
											"FATAL ERROR: the first line of input should be two integers separated by a space. Instead, got: ")
									.append(s1).toString());
					System.exit(1);
				}
				try {
					...
					System.err.println((new StringBuilder()).append(
							"FATAL ERROR: invalid map dimensions: ").append(s1)
							.toString());
					System.exit(1);
				}
				...
				do {
					...
					case '1': // '1'
						if (flag) {
							System.err
									.println((new StringBuilder())
											.append(
													"FATAL ERROR: found two locations for player 1 in the map! First location is (")
											.append(
													getPlayerOneLocation()
															.getX())
											.append(",").append(
													getPlayerOneLocation()
															.getY()).append(
					...
					case '2': // '2'
						if (flag1) {
							System.err
									.println((new StringBuilder())
											.append(
													"FATAL ERROR: found two locations for player 2 in the map! First location is (")
											.append(
													getPlayerTwoLocation()
															.getX())
											.append(",").append(
													getPlayerTwoLocation()
															.getY()).append(
					...
					default:
						System.err
								.println((new StringBuilder())
										.append(
												"FATAL ERROR: invalid character received. ASCII value = ")
										.append(i1).toString());
						System.exit(1);
						break;
					}
				...
				if (j != width * height) {
					System.err
							.println((new StringBuilder())
									.append(
											"FATAL ERROR: wrong number of spaces in the map. Should be ")
									.append(width * height).append(
											", but only found ").append(j)
