 ...
 * 
 */
public class NearMinMax implements TronBot {
	...
	private int minMax(boolean[][] walls, int width, int height, int mx,
		...
		if (level == 0) {
			// // 配列を複製
			// if (wallsCopy == null) {
			// wallsCopy = new boolean[walls.length][];
			// }
			// copy(walls, wallsCopy);
			// return count(wallsCopy, mx, my, width, height); //
			// 実際は盤面を評価して評価値を決める。ここではとりあえず0にしておく。
	...
	public String makeMove(Map map) {
		...
		double distance = java.awt.Point.distance(mx, my, ox, oy);

		if (distance < 3) {
			System.out.println("Avoid!");
			return makeAvoidanceMove(map);
		}
		// Mini Max
		else if (distance < 10) {

			int c = minMax(map.getWalls(), map.getWidth(), map.getHeight(), mx,
					my, ox, oy, false, DEPTH, DEPTH);
			if (c == 0) {
		...
		// Open destination
			...
			return makeOpenDestinationMove(map);
		}
	}

	public String makeRandomMove(Map m) {
	...
	public String makeOpenDestinationMove(Map map) {
		// 自分の位置を取得
		int mx = map.myX();
		int my = map.myY();
		int ox = map.opponentX();
		...
		// 動ける方向をチェック
		if (!map.isWall(mx, my - 1)) {
			double d = openTiles(map.getWalls(), map.getWidth(), map
					.getHeight(), mx, my - 1, ox, oy);
		...
		if (!map.isWall(mx + 1, my)) {
			double d = openTiles(map.getWalls(), map.getWidth(), map
					.getHeight(), mx + 1, my, ox, oy);
	 ...
	 */
	public String makeAvoidanceMove(Map map) {
