 ...
 * 
 */
public class MinMaxAB implements TronBot {

	private int DEBUG_LEVEL = 0; // small -> big = 0 -> 2
	...
	private void print(boolean[][] walls, int w, int h) {
		if (DEBUG_LEVEL < 2)
			return;
	...
	private int openTiles(boolean[][] walls, int width, int height, int mx,
		...
		d(0, DEPTH, "my : " + myTiles);
		d(0, DEPTH, "op : " + opTiles);
	...
	private void d(int level, int searchLevel, String str) {
		d(level, searchLevel, str, 1);
	}

	private void d(int level, int searchLevel, String str, int dLevel) {
		if (DEBUG_LEVEL < dLevel)
			return;

		for (int i = 0; i < searchLevel - level; i++) {
			str = "  " + str;
		}
	}
	 ...
	 *            探索する最大深さ
	 */
	private int alphaBeta(boolean[][] walls, int width, int height, int mx,
			int my, int ox, int oy, boolean isOpponent, int level,
			int searchLevel, int alpha, int beta) {
		// ノードの評価値
		int value;
		// ミニマックス法で求めた最大の評価値を持つ場所
		int best = 0;

		d(level, searchLevel, "isOppnent : " + isOpponent);
		d(level, searchLevel, "level : " + level);
		d(level, searchLevel, "Alpha, Beta = " + alpha + "," + beta);
		d(level, searchLevel, "mx, my = " + mx + "," + my, -1);
		d(level, searchLevel, "ox, oy = " + ox + "," + oy, -1);

		// ぶつかる場合は-500
		if (mx == ox && my == oy) {
			d(level, searchLevel, "Will Collide!", -1);
			return -500;
		}
		...
		if (level == 0) {
			...
			print(walls, width, height);
			int ot = (isOpponent ? -1 : 1)
					* openTiles(walls, width, height, mx, my, ox, oy);
			d(level, searchLevel, "openTiles : " + ot);
			return ot;
		}
		...
		if (isOpponent) {
			for (int[] dir : DIRECTIONS) {
				d(level, searchLevel, "Direction : " + (char) dir[2]);

				// 衝突する場合は-500に
				if (mx + dir[0] == ox && my + dir[1] == oy) {
					d(level, searchLevel, "Will Collide!", -1);
					return -500;
				}
				...
				if (!isWall(walls, mx + dir[0], my + dir[1], width, height)) {
					...
					// 子ノードの評価値を計算（再帰）
					// 今度は相手の番なのでflagが逆転する
					int cVal = alphaBeta(walls, width, height, ox, oy, mx
							+ dir[0], my + dir[1], !isOpponent, level - 1,
							searchLevel, alpha, beta);
					d(level, searchLevel, "cVal : " + cVal);
					// 進む前に戻す
					walls[mx + dir[0]][my + dir[1]] = false;
			...
			value = beta;
			d(level, searchLevel, "- Alpha, Beta = " + alpha + "," + beta);
			d(level, searchLevel, "Value : " + value);
			d(level, searchLevel, "best : " + (char) best);
	...
	public String makeMove(Map map) {
		...
		int c = alphaBeta(mapCopy, map.getWidth(), map.getHeight(), mx, my, ox,
				oy, false, DEPTH, DEPTH, Integer.MIN_VALUE, Integer.MAX_VALUE);
